...
def fetch_urlhaus():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️ URLHaus returned status {response.status_code}")
    except Exception as e:
        print(f"⚠️ Error fetching from URLHaus: {e}")
    return []
...
def fetch_phishtank():
    ...
    try:
        ...
        print(f"⚠️ Error fetching from PhishTank: {e}")
    return []
...
def fetch_malwarebazaar():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️ MalwareBazaar returned status {response.status_code}")
    except Exception as e:
        print(f"⚠️ Error fetching from MalwareBazaar: {e}")
    return []
